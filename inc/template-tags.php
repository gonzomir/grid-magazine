...
if ( ! function_exists( 'grid_mag_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function grid_mag_posted_on() {
...
if ( ! function_exists( 'grid_mag_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function grid_mag_entry_footer() {
 ...
 * @return bool
 */
function grid_mag_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'grid_mag_categories' ) ) ) {
		...
		set_transient( 'grid_mag_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so grid_mag_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so grid_mag_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in grid_mag_categorized_blog.
 */
function grid_mag_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'grid_mag_categories' );
}
